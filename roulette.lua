...
function gg.com(command) --Выполнить команду
...
function gg.money(nick) --Баланс игрока
    local c = gg.com("money " .. nick) 
...
function gg.checkMoney(nick,money) --Чекнуть, баланс, если хватает, то снять бабки
    local balance = gg.money(nick)
    balance = string.sub(balance, 1, string.len(balance) - 3)
    if string.find(balance, "-") ~= nil then
        return false
    else
        balance = string.gsub(balance,",","")
        if tonumber(balance) < money then
            return false
        else
            gg.com("money take " .. nick .. " " .. money)
            return true
        end
    end
end
...
gpu.set(103,15,"ЛКМ 1 ЭМ")
gpu.set(103,16,"ПКМ 10 ЭМ")
gpu.setForeground(0x777777)
gpu.set(103,18,"Казино:")
gpu.set(103,19,"McSkill")
...
while true do
  ...
  while endbets==0 or (endbets>os.time()) do 
    ...
    if(p~=nil) then
      ...
      if(fixClicks(left,top)) then
        ...
        else localsay("§c"..p..", у вас нет столько денег.") end
      end
    end
  end
  ...
  for p,money in pairs(Bets[out]) do gg.com("money give " .. p .." ".. money) localsay(p.." выиграл "..money) end
end